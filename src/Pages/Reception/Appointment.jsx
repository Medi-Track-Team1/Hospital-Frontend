...
if (!editAppointment.patientName.trim()) errors.patientName = 'Patient name is required';
...
if (!editAppointment.department) errors.department = 'Department is required';
if (!editAppointment.phone.trim()) errors.phone = 'Phone number is required';
if (!editAppointment.email.trim()) errors.email = 'Email is required';
if (!editAppointment.age || editAppointment.age < 1) errors.age = 'Valid age is required';
if (!editAppointment.symptoms.trim()) errors.symptoms = 'Symptoms/reason is required';
...
// Phone validation (basic)
const phoneRegex = /^[\+]?[1-9][\d]{0,15}$/;
if (editAppointment.phone && !phoneRegex.test(editAppointment.phone.replace(/[\s\-\(\)]/g, ''))) {
  errors.phone = 'Please enter a valid phone number';
}
...
<stat.icon size={24} style={{ color: stat.color.split(',')[0].split('(')[1] }} />
  ...
  onChange={(e) => {
    ...
    // Only allow numbers, and limit to max 3 digits
    if (/^\d{0,3}$/.test(value)) {
      setNewAppointment(prev => ({ ...prev, age: value }));
    }
  }}
  style={{
    padding: '10px 14px',
...
/>
  ...
  onChange={(e) => {
    ...
    if (digitsOnly.length <= 10) {
      setNewAppointment(prev => ({ ...prev, phone: digitsOnly }));
    }
  }}
  style={{
    padding: '10px 14px',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          <strong style={{ color: '#1e293b' }}>{appointmentToDelete.patientName}</strong>?
          <br />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ color: '#64748b', fontSize: '14px' }}>Doctor:</span>
            <span style={{ color: '#1e293b', fontSize: '14px', fontWeight: '600' }}>
              {appointmentToDelete.doctor}
            </span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: '#64748b', fontSize: '14px' }}>Date & Time:</span>
            <span style={{ color: '#1e293b', fontSize: '14px', fontWeight: '600' }}>
