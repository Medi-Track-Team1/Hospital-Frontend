    ...
    <div className="min-h-screen bg-blue-100 pt-28 px-6 flex flex-col items-center">
      ...
      <div className="flex flex-col lg:flex-row bg-blue-100 mt-6 p-6 mb-10 w-full max-w-6xl items-center">
        ...
        <div className="lg:w-1/2 lg:pl-8 text-black space-y-4">
          ...
          <p className="animate-fade-in-up">Our Hepatology department provides specialized care for liver diseases with a team of experienced hepatologists and dedicated support staff.</p>
          <p className="animate-fade-in-up">We address a wide spectrum of liver-related conditions, including viral hepatitis, liver fibrosis, autoimmune liver disease, and hepatocellular carcinoma.</p>
          <p className="animate-fade-in-up">From early diagnosis to liver transplant evaluation, we ensure personalized treatment paths tailored to each patient’s health status.</p>
          <p className="animate-fade-in-up">The department is equipped with cutting-edge imaging, liver biopsy services, and multidisciplinary support to offer holistic care.</p>
          <p className="animate-fade-in-up">Our mission is to improve liver health outcomes with compassion, advanced science, and continuous support.</p>
        </div>
      </div>
        ...
        {doctors.map((doc) => (
          <div key={doc.id} className="bg-white p-4 rounded-xl shadow-md w-[450px] h-[520px] flex flex-col items-center animate-fade-in-up">
            ...
            <div className="mt-4 text-center">
              ...
              <p className="text-blue-600 text-sm">Hepatologist</p>
              <div className="flex justify-center items-center text-yellow-500 text-sm mt-1">
                {"★".repeat(Math.floor(doc.rating)) + (doc.rating % 1 ? "☆" : "")}<span className="text-black ml-2">{doc.rating}</span>
              </div>
            </div>
